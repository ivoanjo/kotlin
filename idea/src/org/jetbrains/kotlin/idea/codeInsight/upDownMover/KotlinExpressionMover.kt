...
class KotlinExpressionMover : AbstractKotlinUpDownMover() {
    ...
    private fun getValueParamOrArgTargetRange(
        ...
        var next: PsiElement? = sibling
        if (next!!.node.elementType === KtTokens.COMMA) {
    ...
    override fun checkAvailable(
        ...
        val firstElement = getMovableElement(psiRange.getFirst(), false)
        var lastElement = getMovableElement(psiRange.getSecond(), true)
    ...
    companion object {
        ...
        private fun extendForSiblingComments(
            ...
            var currentEnd = end
            if (!(currentStart === currentEnd && currentStart === sibling)) {
                return null
            }
